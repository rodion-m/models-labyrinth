...
8. Run the completion gate below before every recommendation. Validate a small JSON decision artifact before rendering prose; there is no model-only shortcut.
...
| Complex cross-field comparison | Run `scripts/select-models.mjs` with a cache directory | It validates one bundle, reuses matching hashes, parses once, and can calculate a task-specific score from explicitly selected lanes. |
