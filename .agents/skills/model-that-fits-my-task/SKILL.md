...
8. Return a Pareto decision. If no candidate dominates and ranking weights are necessary, expose them and check whether reasonable reweighting changes the winner.
9. Run the completion gate below before every recommendation. Validate a small JSON decision artifact before rendering prose; there is no model-only shortcut.
