...
8. For “best quality at the lowest price,” run `--pareto quality-cost` after the quality gate and return the whole front unless a budget or quality threshold selects one point.
...
10. Run the completion gate below before every recommendation. Validate a small JSON decision artifact before rendering prose; there is no model-only shortcut.
...
For full-snapshot work, use the bundled downloader or selector; do not fetch health, schema, and snapshot independently. Its `content_hash` is the evidence identity. High-stakes choices still require organization-specific validation and qualified review.
